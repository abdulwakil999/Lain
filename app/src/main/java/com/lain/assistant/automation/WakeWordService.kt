...
class WakeWordService : Service() {
    ...
    override fun onCreate() {
        ...
        if (!hasMicPermission()) {
            VoiceSession.enter(VoiceState.ERROR, "Microphone permission isn't granted")
            stopSelf()
            return
        }
        ...
        scope.launch {
            VoiceSession.state.collect { state ->
                when (state) {
                    // Only stand down when the recogniser genuinely needs the
                    // microphone. When it is reading our pipe instead, standing down
                    // would close the very stream it is reading.
                    VoiceState.LISTENING_FOR_COMMAND ->
                        if (detector?.isFeedingCommand != true) detector?.suspendDetection()
        ...
        if (isScreenOn()) resume() else VoiceSession.enter(VoiceState.IDLE)
    }
    ...
    private fun resume() {
        ...
        d.start()
        d.resumeDetection()
        VoiceSession.enter(VoiceState.LISTENING_FOR_WAKE_WORD)
    }

    private fun pause() {
        detector?.suspendDetection()
        if (VoiceSession.state.value == VoiceState.LISTENING_FOR_WAKE_WORD) {
            VoiceSession.enter(VoiceState.IDLE)
        }
    }
    ...
    private fun restartRoute() {
        if (VoiceSession.state.value != VoiceState.LISTENING_FOR_WAKE_WORD) return
        scope.launch {
            detector?.suspendDetection()
            delay(300)
            if (isScreenOn() && !micTakenByOther) detector?.resumeDetection()
    ...
    private fun onWakeWord(heard: String) {
        ...
        if (engine == null) {
            ...
            VoiceSession.enter(VoiceState.ERROR, "Lain isn't running")
        ...
        val was = VoiceSession.state.value
        if (was == VoiceState.SPEAKING || was == VoiceState.PROCESSING || was == VoiceState.EXECUTING) {
            engine.silence()
        }

        VoiceSession.enter(VoiceState.WAKE_WORD_DETECTED)
        ...
        if (!carried.isNullOrBlank()) {
            ...
            // The command is recognised from the stream already open, where the
            // platform allows it. That is what stops the microphone being handed
            // over and back on every turn — and stops the indicator blinking.
            engine.startVoiceInput(
                audioSource = detector?.openCommandStream(),
                onSourceFinished = { detector?.closeCommandStream() }
            )
        }
    ...
    private fun buildNotification(): android.app.Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setContentTitle("Waiting for you to say Lain")
            // States exactly what is happening, because a microphone notification
            // that is vague about it is worse than none.
            .setContentText("Nothing is recorded or sent. Lain is only listening for her name.")
            .setStyle(
                NotificationCompat.BigTextStyle().bigText(
                    "Say \"Lain\" — \"hello Lain\", \"hey Lain\", \"sup Lain\", or just her name.\n\n" +
                        "Nothing is recorded, saved or sent anywhere. Until she hears her name, the " +
                        "audio never leaves this phone and is never turned into words — it is " +
                        "measured for loudness and thrown away, frame by frame.\n\n" +
                        "The microphone stays open the whole time hands-free is on, in one steady " +
                        "stream. That is deliberate: opening and closing it for every sound is what " +
                        "made the mic indicator blink and cost battery. A lit indicator here means " +
                        "the stream is open, not that anything is being kept.\n\n" +
