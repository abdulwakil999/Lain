...
class WakeWordDetector(
    ...
    /**
     * The command recogniser's end of this same stream, while a turn is running.
     *
     * The whole point of it. Handing the command over used to mean closing the
     * recorder, letting the recogniser open the microphone, and opening ours again
     * afterwards — one blink of the privacy indicator per turn, plus two audio route
     * changes and a window where neither owns it. Writing into a pipe instead means
     * the microphone is opened when hands-free goes on and closed when it goes off,
     * and nothing in between touches it.
     */
    @Volatile private var commandSink: ParcelFileDescriptor.AutoCloseOutputStream? = null
    ...
    fun stop() {
        ...
        VoiceSession.releaseMicrophone(VoiceSession.OWNER_WAKE_WORD)
    }

    /** Held after a wake and after each reply starts, so she doesn't answer herself. */
    suspend fun cooldown() = delay(COOLDOWN_MS)

    // ------------------------------------------------- the turn, on the same stream

    /**
     * Hands out the read end of this stream so a command can be recognised from it.
     *
     * Android 13 and up only, and null on anything else — below that a recogniser
     * can read nothing but the microphone, so the caller has to take it. Null is
     * also the answer when the recorder is not actually open, because a pipe nobody
     * is writing to is a recogniser that waits forever.
     *
     * @return the descriptor to hand the recogniser, or null to open the mic instead.
     */
    fun openCommandStream(): ParcelFileDescriptor? {
        if (!canFeedCommand()) return null
        if (!running || suspended || recorder == null) return null
        closeCommandStream()

        val pipe = runCatching { ParcelFileDescriptor.createPipe() }.getOrNull() ?: return null
        commandSink = ParcelFileDescriptor.AutoCloseOutputStream(pipe[1])
        return pipe[0]
    }

    /**
     * Whether a command can be recognised from this stream rather than the mic.
     *
     * The pipe API arrived in Android 13. [pipeWorks] is the other half: a recogniser
     * that quietly ignores a piped source has already proved it during verification,
     * and there is no sense finding out again with the user waiting.
     */
    fun canFeedCommand(): Boolean =
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU && pipeWorks

    /** True while a turn is reading this stream. */
    val isFeedingCommand: Boolean get() = commandSink != null

    /** Ends the turn's read of this stream. The recorder is untouched, deliberately. */
    fun closeCommandStream() {
        commandSink?.let { runCatching { it.close() } }
        commandSink = null
    }

    /**
     * One frame into the command pipe.
     *
     * A failed write means the recogniser has closed its end — it has decided, and
     * the result is already on its way to the caller through the listener. Nothing
     * to report, just stop writing.
     */
    private fun writeCommandFrame(
        sink: ParcelFileDescriptor.AutoCloseOutputStream,
        frame: ShortArray,
        count: Int
    ) {
        ...
        runCatching { sink.write(bytes) }.onFailure { closeCommandStream() }
    }
    ...
    private suspend fun run() {
        ...
        while (scope.isActive && running) {
            ...
            if (recorder == null) {
                if (!VoiceSession.claimMicrophone(VoiceSession.OWNER_WAKE_WORD)) {
                    delay(400)
                    continue
                }
                if (!openRecorder()) {
            ...
            // A command turn is reading this stream. Same frames, same recorder —
            // the recogniser is simply on the other end of a pipe.
            commandSink?.let { sink -> writeCommandFrame(sink, frame, read) }
                ...
                verdict.getAndSet(null)?.let { result ->
                    ...
                    when {
                        result.text.isNotEmpty() && isWakePhrase(result.text) -> {
                            pipeFailures = 0
                            refractory = BASE_REFRACTORY_MS
                            VoiceSession.recordWake()
                            if (!canFeedCommand()) {
                                // No pipe on this device, so the command recogniser has
                                // to have the microphone. Closed here, synchronously,
                                // before anyone is told: setting `suspended` only makes
                                // the loop close it on its *next* pass, and the command
                                // recogniser starts long before that — finding this
                                // AudioRecord still on the microphone and failing with
                                // ERROR_RECOGNIZER_BUSY. Which is a wake word that works
                                // followed by an assistant that hears nothing.
                                suspended = true
                                closeRecorder()
                            }
    ...
    private fun listener() = object : RecognitionListener {
        ...
        override fun onError(error: Int) {
            VoiceSession.recordDiagnostic("recogniser error $error")
    ...
    private fun isWakePhrase(heard: String): Boolean {
        ...
        val said = VoiceSession.spokenAloud.lowercase()
        if (said.isNotEmpty()) {
